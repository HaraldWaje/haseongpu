...
x_center= variable_data['x_center']
y_center= variable_data['y_center']
surface= variable_data['surface']
normals_p= variable_data['normals_p']
forbidden=variable_data['forbidden']

beta_cell = np.zeros((len(p), mesh_z))
beta_vol = np.zeros((len(p), mesh_z-1))
nT, b = ordered_int.shape                
reflectivities = np.zeros((1, nT*2))

# initialize arrays
phi_ASE = np.zeros((len(p), mesh_z))
...
crystal['tfluo'] = 1
beta_c_2 = np.zeros((len(p), mesh_z))
intensity = pump['I']
